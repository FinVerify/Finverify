"""Tests for the additive /v1/verify verification-status contract."""
...
def test_v1_verify_scale_mul100_profit_margin(client):
    data = client.post("/v1/verify", json={"question": "What was the profit margin?", "raw_value": 0.2531}).json()
    assert_unverified(data)
...
def test_v1_verify_cet1_ratio_no_correction(client):
    data = client.post("/v1/verify", json={"question": "What was the CET1 ratio?", "raw_value": 10.935}).json()
    assert_unverified(data)
...
def test_v1_verify_htm_decrease_no_correction(client):
    data = client.post("/v1/verify", json={"question": "What was the percentage decrease in HTM securities?", "raw_value": -34.11}).json()
    assert_unverified(data)
    assert data["correction_applied"] is None
...
def test_v1_verify_pe_ratio_no_correction(client):
    data = client.post("/v1/verify", json={"question": "What was the price to earnings ratio?", "raw_value": 28.5}).json()
    assert_unverified(data)
    assert data["correction_applied"] is None
...
def test_v1_verify_revenue_growth_scale_mul100(client):
    data = client.post("/v1/verify", json={"question": "What was the revenue growth rate?", "raw_value": 0.0623}).json()
    assert_unverified(data)
    assert "scale_mul100" in data["correction_applied"]
...
def test_v1_verify_missing_field_returns_422(client):
    assert client.post("/v1/verify", json={"question": "What was revenue?"}).status_code == 422
